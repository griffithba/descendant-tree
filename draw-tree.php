...
function drawTree(&$blocks, &$targetObject, &$baseObject) {
           ...
           if (($grid[$i][$j]->person != null) && ($grid[$i][$j]->person == $grid[$i][$j+$grid[$i][$j]->colspan]->person)) {
               ...
               if($grid[$i][$j]->colspan > $grid[$i-4][$j]->colspan) { // if child span is bigger than parent span
                   ...
                   for ($k=1; $k<$grid[$i][$j]->colspan; ) {
                       if ($j + $k + $grid[$i-2][$j+$k]->colspan < $j + $grid[$i][$j]->colspan) {
   ...
   for ($i=5; $i<$depth*4; $i+=4) {  // start with 2nd row of 2nd generation (contains names)
       for ($j=0; $j+$grid[$i][$j]->colspan < $width; ) {
           // if name cell of this generation is bigger than name cell of previous generation
           if ($grid[$i][$j]->colspan < $grid[$i-4][$j]->colspan) { // (if child span is smaller than parent span)
               ...
               for ($k=1; $k<$grid[$i-3][$j]->colspan; ) {
                   ...
                   if ($j + $k + $grid[$i-2][$j+$k]->colspan < $j + $grid[$i-3][$j]->colspan) {
                       $grid[$i-2][$j+$k]->text = "<hr width=\"100%\">";  // middle cell
                   } else {
                       $grid[$i-2][$j+$k]->align = "left";
                       $grid[$i-2][$j+$k]->text = "<hr width=\"50%\" align=\"left\">";  //last cell
                   }
...
function putBlockHere(&$grid, &$blocks, &$thisBlock, $generation, $left, $width) {
   ...
   for ($i=($generation-1)*4, $j=0; $j<count($thisBlock); $i+=4, $j++){
       ...
       if (!$thisBlock[$j]->isBase) {  // If this is not the bottom person
           // Print vertical lines in bottom two cells to connect to children
           $grid[$i+2][$left]->text = "|";
           $grid[$i+3][$left]->text = "|";
       }
...
// Function to lay out tree -- this doesn’t need to be a function
